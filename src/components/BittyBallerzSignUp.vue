  ...
  <div class="camp-landing">
    ...
    <section class="hero">
      ...
      <div class="hero__content">
        <div class="hero__eyebrow">Itty Bitty Ballers 2026 Fall Academy Series</div>
        <h1 class="hero__headline">
          <span class="hero__headline--stroke">ELEVATE</span>
          <span class="hero__headline--solid">YOUR GAME</span>
        </h1>
        <img class="hero__camp-logo" :src="getImageUrl('bittyballers_logo_sm-transparent','png')" alt="bitty ballers logo" width="200px" height="200px">
        <p class="hero__sub">
          Big Fun, Small Hoops · All Fun, No Pressure · Where Little Legends Learn to Ball.<br />
          For ages 5–7
        </p>
        ...
        >
          ...
          <div class="reg-form__row">
            ...
            <div class="form-group" :class="{ 'form-group--error': errors.age }">
              ...
              <input id="age" v-model="form.age" name="Player Age" type="number" min="5" max="7" placeholder="5" />
            ...
            <div class="form-group" :class="{ 'form-group--error': errors.session }">
              ...
              <select id="session" v-model="form.session" name="Session">
                ...
                <option
                  ...
                  :key="s.id"
                  :value="s.id"
                  :disabled="!isSessionEligible(s)"
                >
                  {{ s.name }} — {{ s.dates }} (Ages {{ s.ages }}){{ !isSessionEligible(s) && form.plan ? ' — Not eligible' : '' }}
                </option>
              </select>
            ...
            <div class="form-group" :class="{ 'form-group--error': errors.plan }">
              ...
              <select id="plan" v-model="form.plan" name="Package">
                ...
                <option
                  ...
                  :key="p.id"
                  :value="p.id"
                  :disabled="!isPlanEligible(p)"
                >
                  {{ p.name }} — ${{ p.price }}{{ !isPlanEligible(p) && form.session ? ' — Not eligible' : '' }}
                </option>
              </select>
          ...
          <!-- Contact banner — shown when the selected session and package do not match -->
          <div v-if="isSelectionIneligible" class="reg-form__contact">
            <div class="reg-form__contact-icon">🏀</div>
            <div>
              <strong>This session and package cannot be combined.</strong>
              <p>Select the package that matches your session, or reach out and we'll help you find the right option.</p>
...
const sessions = ref([
  {
    id: 'session-a',
    week: '',
    name: 'Itty Bitty Ballers Drop In',
    dates: '1 Single Session of Itty Bitty Ballers Academy',
    days: ['Saturdays', '12:00 AM – 1:00 PM', 'Challenger Elementary Gym'],
    ages: '5–7',
    featured: false,
  },
  {
    id: 'session-b',
    week: '',
    name: 'Itty Bitty Ballers Academy',
    dates: 'October 3rd, October 17th, October 31st, November 14',
    days: ['Saturdays', '12:00 AM – 1:00 PM', 'Challenger Elementary Gym'],
    ages: '5–7',
    featured: true,
  }
...
// A selected session and package are compatible only when their flags match.
function isSessionEligible(session) {
  const selectedPlan = getPlan(form.plan)
  return !selectedPlan || session.featured === selectedPlan.highlight
}

function isPlanEligible(plan) {
  const selectedSession = getSession(form.session)
  return !selectedSession || plan.highlight === selectedSession.featured
}

const isSelectionIneligible = computed(() => {
  const selectedSession = getSession(form.session)
  const selectedPlan = getPlan(form.plan)
  return !!selectedSession && !!selectedPlan && selectedSession.featured !== selectedPlan.highlight
})

// Select the package whose highlight flag matches the selected session.
watch(() => form.session, (sessionId) => {
  ...
  const matchingPlan = plans.value.find((plan) => plan.highlight === selectedSession.featured)
  form.plan = matchingPlan?.id ?? ''
})
...
function validate() {
  ...
  if (!form.age) {
    errors.age = 'Please select the player\'s age.'
  } else if (!Number.isInteger(age) || age < 5 || age > 7) {
    errors.age = 'The player must be between 5 and 7 years old.'
  }
  ...
  if (!form.plan) {
    errors.plan = 'Please select a package.'
  } else if (!isPlanEligible(getPlan(form.plan))) {
    errors.plan = 'This package does not match the selected session.'
  }
...
.btn--full { width: 100%; }
.btn--primary { background: var(--black-mid); color: #fff; }
.btn--primary:hover { background: var(--orange-dark); border: 1px solid var(--white); }
.btn--ghost {
  background: var(--blue);
  color: var(--white);
  border: 2px solid rgba(255,255,255,0.35);
}
.btn--ghost:hover { border-color: var(--blue); color: var(--black-mid); background-color: var(--white); }
...
.hero__grid-lines {
  position: absolute; inset: 0;
  background-image:
    linear-gradient(rgba(79,156,225,0.25) 1px, transparent 1px),
    linear-gradient(90deg, rgba(79,156,225,0.25) 1px, transparent 1px);
